...
fareprim <- function(data, id.var, time.var, x.vars, y.vars, w.vars = NULL, p.vars = NULL, 
  ...
  scaled = FALSE, by.id = NULL, by.year = NULL, shadow = FALSE) {
...
We recommend rescaling the data or set the scaled option to TRUE to avoid numerical problems\n\r", call. = FALSE)
    ...
    res.fp.loop <- foreach(ano = 1:(length(year.vec)), .combine = rbind, .packages = c("lpSolveAPI", 
      "doParallel"), .export = c("fp.1", "DO.sh", "DI.sh", "DO.ome", "DI.ime", "D.tfp")) %dopar% 
      {
        fp.1(data, data.in, step1, ano, year.vec, tech.reg, rts, orientation, parallel, scaled, 
             PRICESO = P.fp, PRICESI = W.fp, itt, it)
      }
  } else {
  ...
  fp.data <- cbind(fp.data, res.fp.loop)
    ...
    if (!(is.null(by.id)) & !(is.null(by.year))) {
      if (by.id > length(id.vec)) 
        stop("by.id is out of range: by.id must be lower or equal to ", paste(length(id.vec)), .call = FALSE)
      if (by.year > length(id.vec)) 
        stop("by.year is out of range: by.year must be  lower or equal to ", paste(length(year.vec)), .call = FALSE)
      ...
      if (!(is.null(by.id)) & (is.null(by.year))) {
        ...
        if ((is.null(by.id)) & !(is.null(by.year))) {
          ...
          {
            fp.data[fp.data[, 2] == ano, names.ind]/fp.data[fp.data[, 2] == year.vec[by.year], names.ind]
          }
        } else {
          indices.1 <- foreach(ano = year.vec, .combine = rbind, .packages = c("doParallel")) %dopar% 
          {
            fp.data[fp.data[, 2] == ano, names.ind]/fp.data[fp.data[, 2] == year.vec[1], names.ind]
          }
        }
      }
    }
  ...
  stopImplicitCluster()
    ...
    if (shadow == TRUE) {
      ...
      res.tfp <- list(Levels = fp.data, Changes = indices.2)
      }
