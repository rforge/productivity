...
fisher <- function(data, id.var, time.var, x.vars, y.vars, w.vars, p.vars, tech.change = TRUE, 
  ...
  parallel = FALSE, cores = max(1, detectCores() - 1), scaled = FALSE, shadow = FALSE) {
...
We recommend rescaling the data or set the scaled option to TRUE to avoid numerical problems\n\r", call. = FALSE)
  ...
  if (tech.change == TRUE) {
    res.fish.loop <- foreach(ano = 1:(length(year.vec)), .combine = rbind, .packages = c("lpSolveAPI", 
      ...
      fish.1(data, data.in, step1, ano, year.vec, tech.reg, rts, orientation, parallel, scaled, itt, it)
    }
  } else {
    res.fish.loop <- foreach(ano = 1:(length(year.vec)), .combine = rbind, .packages = c("lpSolveAPI", 
      "doParallel"), .export = c("fish.2", "DO.sh", "DI.sh", "DO.ome", "DI.ime", "D.tfp", 
      "DO.shdu", "DI.shdu")) %dopar% {
      fish.2(data, data.in, step1, ano, year.vec, rts, orientation, parallel, scaled, itt, it)
    }
  }
  ...
  stopImplicitCluster()
    ...
    INT <- matrix(nrow = dim(res.fish.loop2[res.fish.loop2[, 2] == year.vec[1], ])[1], ncol = 18)
    ...
    if (shadow == TRUE) {
      ...
      res.tfp <- list(Levels = fish.data, Changes = indices.2)
      }
