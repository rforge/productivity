## Malmquist-nt with technical change

malm.nt.1 <- function(data, data.in, step1, ano, year.vec, tech.reg, rts, orientation, parallel, scaled) {
  ## period (Xt1, Yt1)
  ...
  res2 <- foreach(dmu = 1:length(data[data[, step1$time.var] == year.vec[ano], step1$id.var]), .combine = rbind, 
    ...
    PRICESO <- DO.shdu(XOBS = X1[, dmu], YOBS = Y2[, dmu], XREF = XREF1, YREF = YREF1, rts)
    PRICESI <- DI.shdu(XOBS = X2[, dmu], YOBS = Y1[, dmu], XREF = XREF1, YREF = YREF1, rts)
...
## Malmquist-nt without technical change

malm.nt.2 <- function(data, data.in, step1, ano, year.vec, rts, orientation, parallel, scaled) {
  ## period (Xt1, Yt1)
  X1 <- t(as.matrix(data[data[, step1$time.var] == year.vec[ano], step1$x.vars]))
  Y1 <- t(as.matrix(data[data[, step1$time.var] == year.vec[ano], step1$y.vars]))
  ## period (Xt, Yt)
  ...
  res2 <- foreach(dmu = 1:length(data[data[, step1$time.var] == year.vec[ano], step1$id.var]), .combine = rbind, 
    ...
    PRICESO <- DO.shdu(XOBS = X1[, dmu], YOBS = Y2[, dmu], XREF = XREFs, YREF = YREFs, rts)
    PRICESI <- DI.shdu(XOBS = X2[, dmu], YOBS = Y1[, dmu], XREF = XREFs, YREF = YREFs, rts)
...
### Malmquist - NT, print fonction
print.MalmquistNT <- function(x, digits = NULL, ...) {
  if (is.null(digits)) {
    digits <- max(3, getOption("digits") - 3)
  }
  cat("\nMalmquist-NT productivity and profitability levels (summary):\n\n")
  print(summary(x[["Levels"]], digits = digits), digits = digits)
  cat("\n\nMalmquist-NT productivity and profitability changes (summary):\n\n")
  print(summary(x[["Changes"]], digits = digits), digits = digits)
  cat("\n\nMalmquist-NT productivity shadow prices (summary):\n\n")
