...
paasche <- function(data, id.var, time.var, x.vars, y.vars, w.vars, p.vars, tech.change = TRUE, 
  ...
  if (tech.change == TRUE) {
    res.paa.loop <- foreach(ano = 1:(length(year.vec)), .combine = rbind, .packages = c("lpSolveAPI", 
      ...
      paa.1(data, data.in, step1, ano, year.vec, tech.reg, rts, orientation, parallel, scaled, itt, it)
    }
  } else {
    res.paa.loop <- foreach(ano = 1:(length(year.vec)), .combine = rbind, .packages = c("lpSolveAPI", 
      "doParallel"), .export = c("paa.2", "DO.sh", "DI.sh", "DO.ome", "DI.ime", "D.tfp", 
      "DO.shdu", "DI.shdu")) %dopar% {
      paa.2(data, data.in, step1, ano, year.vec, rts, orientation, parallel, scaled, itt, it)
    }
  }
  ...
  stopImplicitCluster()
    ...
    if (shadow == TRUE) {
      ...
      res.tfp <- list(Levels = paa.data, Changes = indices.2)
      }
