...
We recommend rescaling the data or set the scaled option to TRUE to avoid numerical problems\n\r", call. = FALSE)
  ...
  if (tech.change == TRUE) {
    res.las.loop <- foreach(ano = 1:(length(year.vec)), .combine = rbind, .packages = c("lpSolveAPI", 
      ...
      las.1(data, data.in, step1, ano, year.vec, tech.reg, rts, orientation, parallel, scaled, itt, it)
    }
  } else {
    res.las.loop <- foreach(ano = 1:(length(year.vec)), .combine = rbind, .packages = c("lpSolveAPI", 
      "doParallel"), .export = c("las.2", "DO.sh", "DI.sh", "DO.ome", "DI.ime", "D.tfp", 
      "DO.shdu", "DI.shdu")) %dopar% {
      las.2(data, data.in, step1, ano, year.vec, rts, orientation, parallel, scaled, itt, it)
    }
  }
  ...
  stopImplicitCluster()
    ...
    if (shadow == TRUE) {
      ...
      res.tfp <- list(Levels = las.data, Changes = indices.2)
      }
