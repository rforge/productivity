...
We recommend rescaling the data or set the scaled option to TRUE to avoid numerical problems\n\r", call. = FALSE)
    ...
    res.lo.loop <- foreach(ano = 1:(length(year.vec)), .combine = rbind, .packages = c("lpSolveAPI", 
      "doParallel"), .export = c("lo.1", "DO.sh", "DI.sh", "DO.ome", "DI.ime", "D.tfp", 
        ...
        lo.1(data, data.in, step1, ano, year.vec, tech.reg, rts, orientation, parallel, scaled,
             PRICESO = P.lo, PRICESI = W.lo, itt, it)
      }
  } else {
    ...
    if (!(is.null(by.id)) & !(is.null(by.year))) {
      ...
      if (!(is.null(by.id)) & (is.null(by.year))) {
        ...
        if ((is.null(by.id)) & !(is.null(by.year))) {
          ...
          {
            lo.data[lo.data[, 2] == ano, 3:20]/lo.data[lo.data[, 2] == year.vec[by.year], 3:19]
          }
        } else {
          indices.1 <- foreach(ano = year.vec, .combine = rbind, .packages = c("doParallel")) %dopar% 
          {
            lo.data[lo.data[, 2] == ano, 3:20]/lo.data[lo.data[, 2] == year.vec[1], 3:20]
          }
        }
      }
    }
  ...
  stopImplicitCluster()
    ...
    if (shadow == TRUE) {
      ...
      res.tfp <- list(Levels = lo.data, Changes = indices.2)
      }
